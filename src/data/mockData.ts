...
export const labs: Lab[] = [
  { id: 'lab-1', name: 'Lab Komputer', capacity: 36, facilities: ['PC 36 Unit', 'Proyektor', 'AC', 'Whiteboard'], location: 'Gedung A Lt.2', status: 'available' },
];
...
export function checkRoutineConflict(labId: string, dayIndex: number, startHour: number, endHour: number): Schedule | null {
  return schedules.find(s =>
    s.lab_id === labId &&
    s.day === dayIndex &&
    s.start_hour < endHour &&
    s.end_hour > startHour
  ) ?? null;
}

export function checkBookingConflict(labId: string, date: string, startHour: number, endHour: number): Booking | null {
